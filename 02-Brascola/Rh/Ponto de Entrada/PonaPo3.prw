...
If DbSeek(xFilial("SPC")+SRA->RA_MAT)
	If cBH == 'S'//Regime BH
		While !Eof() .AND. SPC->PC_MAT == cMat .AND. SPC->PC_DATA <= MV_PAR14  .AND. cBH == 'S'
			...
			//Preenchimento Cod. Informado para os casos de Horas Extras 50%
			...
			//Preenchimento do Motivo de Abono 030-Descontar BH para eventos como atrasos, saídas antecipadas, falta 1/2 período, falta integral.
			...
			//Preenchimento de Motivo de Abono 012-Aniversário
	...
	Else
		While !Eof() .AND. SPC->PC_MAT == cMat .AND. SPC->PC_DATA <= MV_PAR14 //Procedimento para todos os colaboradores independente do regime
			//Preenchimento de Motivo de Abono 012-Aniversário para colaboradores fora do regime de BH
			If (SPC->PC_PD $ '409') .AND. (Substr(DtoS(SPC->PC_DATA),5,4)  == SubStr(cNasc,5,4))//Faltou porque esta de aniversário, tem que abonar...fmaia
				RecLock("SPC",.F.)
				SPC->PC_ABONO:= '012'
				SPC->PC_QTABONO:= SPC->PC_QUANTC
