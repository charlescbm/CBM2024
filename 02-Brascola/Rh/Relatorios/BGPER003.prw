...
!Nome              ! BGPER003                                                !
+------------------+---------------------------------------------------------+
!Descricao         ! Relatório de CBO de Funcionários                      !
+------------------+---------------------------------------------------------+
...
User Function BGPER003()

Local cPerg   := PADR("BGPER003",10)
Local oReport := BGPER03A(cPerg)

U_BCFGA002("BGPER003")//Grava detalhes da rotina usada
...
Static Function BGPER03A(cPerg)

Local oReport
Local oSection1

oReport:= TReport():New("BGPER003","Relatório de CBO de Funcionários",cPerg,{|oReport| BGPER03C(oReport)},"Impressão de Relatório de CBO de Funcionários")
oReport:SetTotalInLine(.F.)
oReport:nFontBody := 8
     
oSection1 := TRSection():New(oReport,"Relatório de CBO de Funcionários",{"SRA", "SRJ"})  //Tabelas usadas na query
...
//TRCell():New(oSection1,cName         ,cAlias, cTitle        ,cPicture         ,nSize ,lPixel,bBlock,cAlign  ,lLineBreak,cHeaderAlign,lCellBreak,nColSpace,lAutoSize,nClrBack,nClrFore,lBold)
  ...
  TRCell():New(oSection1,"MATRICULA"   ,"SRA" , "Matricula"   ,                 ,  14  ,.F.   ,      ,"CENTER",  .F.     , "CENTER"   ,          ,   1     ,    .F.  ,        ,        ,     )
  TRCell():New(oSection1,"NOME"        ,"SRA" , "Nome"        ,                 ,  60  ,.F.   ,      ,"LEFT"  ,  .F.     , "LEFT"     ,          ,   1     ,    .F.  ,        ,        ,     )  
  TRCell():New(oSection1,"CC"          ,"SRA" , "C.Custo"     ,                 ,  12  ,.F.   ,      ,"CENTER",  .F.     , "CENTER"   ,          ,   1     ,    .F.  ,        ,        ,     )
  TRCell():New(oSection1,"FUNCAO"      ,"SA1" , "Função"      ,                 ,  10  ,.F.   ,      ,"CENTER",  .F.     , "CENTER"   ,          ,   1     ,    .F.  ,        ,        ,     ) 
...
Static Function BGPER03C(oReport)
...
Static Function BGPER03B(cPerg)
