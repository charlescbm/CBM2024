...
//Alert("Você clicou no item Novo do menu...")
//criando uma Dialog
lTransparent := .T.
cFonte := 'Arial'
nTamFonte := 14
...
'<tr>'+;
'<td width=100 bgcolor="#FFFF87">Código'+;
'<td width=200 bgcolor="#FFFF87">Descrição'+;
...
@ 9,  7 BUTTON "&MsgInfo"      SIZE 40,10 OF oTela2 PIXEL ACTION MsgInfo("Informação","Mensagem")
...
@ 39, 7 BUTTON "RetryCancel"   SIZE 40,10 OF oTela2 PIXEL ACTION MsgRetryCancel("Título","Mensagem")
@ 49, 7 BUTTON "&Help 1"       SIZE 40,10 OF oTela2 PIXEL ACTION Help("",1,"","HELP","Help Padrão Protheus",1,0)
...
oList1 := TListBox():New(    001 ,   001  ,{|u|if(Pcount()>0,nList:=u,nList)},   aItems ,     100  ,     100   ,               ,  oTela5,          ,     230    ,            ,     .T.  ,            ,  {||MsgStop("Você deu duplo-clique no item: "+ Str(nList))},         ,            ,            ,          ,            ,            ,            ,            ,            )
*/ 
                                  
	// Cria Componentes Padroes do Sistema
	@ 30,05 LISTBOX oList3 FIELDS HEADER "", "Código" ,"Descrição" PIXEL SIZE 100,80 OF oTela5 ON dblClick(aList3[oList3:nAt,1] := !aList3[oList3:nAt,1],oList3:Refresh())     
