...
±±³Fun‡…o    ³ CAR044   ³ Autor ³ R.H. - Ze Maria       ³ Data ³ 14.03.95 ³±±
...
User Function CAR044(lTerminal,cFilTerminal,cMatTerminal,cMesAnoRef,nRecTipo,cSemanaTerminal)
...
//³ Verifico qual o modulo que esta chamando o relatorio                ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If (cModulo != "GPE")
	MsgInfo("> Utilizar esta rotina para modulo de GPE!","ATENCAO")
	Return
Elseif (cNivel < 6)
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
wnrel:="CAR044"            //Nome Default do relatorio em Disco
IF !lTerminal
...
//nPeriodos  := If(mv_par21 > 0,mv_par21,0)							//Qtde de Periodos
nPeriodos  := If(mv_par23 > 0,mv_par23,0)							//Qtde de Periodos
...
Local cArqMov     := ""
Local aCodBenef   := {}
Local cAcessaSR1  := &("{ || " + ChkRH("CAR044","SR1","2") + "}")
Local cAcessaSRA  := &("{ || " + ChkRH("CAR044","SRA","2") + "}")
Local cAcessaSRC  := &("{ || " + ChkRH("CAR044","SRC","2") + "}")
Local cAcessaSRI  := &("{ || " + ChkRH("CAR044","SRI","2") + "}")
Local cHtml		   := ""	
Local nHoras      := 0
Local nMes, nAno
Private cAliasMov := ""
...
//Monto periodos para processamento
///////////////////////////////////
aPeriodos := {}
dPerAux := dDataRef
For _p := 1 to nPeriodos
	aadd(aPeriodos,dPerAux)
	dPerAux := ctod("01/"+Substr(dtoc(dPerAux),4))-1
Next _p
aSort(aPeriodos)
For _p := 1 to Len(aPeriodos)
                            
	//Periodo referencia
	////////////////////
	dDataRef := aPeriodos[_p]
	cMesAnoRef := StrZero(Month(dDataRef),2)+StrZero(Year(dDataRef),4)
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	cAliasMov := Space(3) ; aOrdBag := {} ; cArqMov := Space(6)
	...
	TOTVENC:= TOTDESC:= FLAG:= CHAVE := SALDOBH := 0
	...
	While SRA->( !Eof() .And. &cInicio <= cFim )
		...
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Salário hora para impressão                   				     |
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		nSalario := 0
		SR3->(dbSetOrder(1))
		SR3->(dbSeek(xFilial("SR3")+SRA->RA_mat,.T.))
		While !SR3->(Eof()).and.(xFilial("SR3") == SR3->R3_filial).and.(SRA->RA_mat == SR3->R3_mat)
			If (mv_par01 >= SR3->R3_data).and.(nSalario < SR3->R3_valor)
		...
		Totvenc := Totdesc := Saldobh := 0
		                        
		//Busco o saldo do banco de horas
		/////////////////////////////////
		SALDOBH := fSaldoBanco()
		...
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Tratamento especifico para facilitar visualizacao            ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		If (mv_par22 == 2).and.(Vez == 0).and.(Esc == 2) //--> Verifica se for FOLHA.
			TrataFaltaAtraso()
		Endif
...
@ Li,30  PSAY STR0077 + Substr(DescFun(SRA->RA_CODFUNC,SRA->RA_FILIAL),1,15)
@ Li,80 PSAY STR0078 + Substr(fDesc("SQ3",SRA->RA_CARGO,"SQ3->Q3_DESCSUM"),1,6)
@ LI,131 PSAY "|"
LI ++
@ LI,00 PSAY "|"+REPLICATE("-",130)+"|"
...
For nCont := 1 To nTermina
   @ LI,00 PSAY "|"
	...
	If nValidos = If(cPaisLoc <> "ARG",15,10)
		@ LI,00 PSAY "|"+REPLICATE("-",130)+"|"
		LI ++
		@ LI,00 PSAY "|"
...
For nCont1 := nValidos+1 To If(cPaisLoc <> "ARG",15,10)
    @ Li,00  PSAY "|"
    @ Li,44  PSAY "|"
    @ Li,88  PSAY "|"
    @ Li,131 PSAY "|"
...
//   @ LI,088 PSAY "|"+STR0032+SPACE(07)+TRANS(TOTDESC,"@E 999,999,999.99")	//" TOTAL DESCONTOS     "
   @ LI,002 PSAY STR0031+SPACE(10)+TRANS(TOTVENC,"@E 999,999,999.99")	//"| TOTAL BRUTO     "
   @ LI,044 PSAY STR0032+SPACE(07)+TRANS(TOTDESC,"@E 999,999,999.99")	//" TOTAL DESCONTOS     "
   @ LI,088 PSAY "|"   
   @ LI,131 PSAY "|"
   LI ++
//   @ LI,000 PSAY "|"
//   @ LI,005 PSAY DESC_MSG2              
   @ LI,000 PSAY "|"+REPLICATE("-",130)+"|"
	...
	LI ++
	@ LI,000 PSAY "|"
	@ LI,003 PSAY DESC_MSG1
	@ LI,044 PSAY "| Salario : "+SPACE(22)+TRANS((nSalario),"@E 9,999.99")			//Salario
//	@ LI,044 PSAY STR0033+SRA->RA_BCDEPSAL+"-"+DescBco(SRA->RA_BCDEPSAL,SRA->RA_FILIAL)	//"| CREDITO:"
	@ LI,088 PSAY STR0034+SPACE(05)+TRANS((TOTVENC-TOTDESC),"@E 999,999,999.99")			//"| LIQUIDO A RECEBER     "
	@ LI,131 PSAY "|"

	LI ++                  
	@ LI,000 PSAY "|"      
	@ LI,003 PSAY DESC_MSG2
	@ LI,044 PSAY "|"+REPLICATE("-",86)+"|"

	LI ++
	@ LI,000 PSAY "|"      
	@ LI,003 PSAY DESC_MSG3
	...
	@ Li,95  PSAY "Saldo Banco de Horas: "+Alltrim(Transform(SALDOBH,"999999.99"))
	@ li,131 PSAY "|"
...
@ LI,00 PSAY "|"+REPLICATE("-",130)+"|"
@ ++LI,00 PSAY "| " + STR0094 + TRANS(TOTVENC,"@E 999,999,999.99") 
@ LI,44 PSAY STR0095 +TRANS(TOTDESC,"@E 999,999,999.99")
@ LI,88 PSAY STR0096 +TRANS((TOTVENC-TOTDESC),"@E 999,999,999.99")
@ LI,131 PSAY "|"
@ ++LI,00 PSAY "|" + REPLICATE("-",130)+"|"
Li ++
...
@ LI,131 PSAY "|"
@ ++Li,00 PSAY STR0085 + TRANS((TOTVENC-TOTDESC),"@E 999,999,999.99")
@ LI,131 PSAY "|"
@ ++Li,00 PSAY STR0086
@ LI,131 PSAY "|"
@ ++Li,00 PSAY "|"                      
...
Static Function fSaldoBanco()
**************************
LOCAL nSaldo := 0
LOCAL dDtIni := SRA->RA_admissa 	//Data Inicial
...
Static Function TrataFaltaAtraso()
******************************
LOCAL _a := 0, _nTam := 0
LOCAL _nPSalario := 0, _nHSalario := 0, _nVSalario := 0
LOCAL _nHFaltas := 0, _nVFaltas := 0, _nHAtraso := 0, _nVAtraso := 0

//Corrige linhas com proventos e descontos
//////////////////////////////////////////
SRV->(dbSetOrder(1))
_nTam := Max(Len(aProve),Len(aDesco))
For _a := 1 to _nTam
	If (_a <= Len(aProve))
		If SRV->(dbSeek(xFilial("SRV")+aProve[_a,1]))
			If (SRV->RV_codfol $ "031/032") //Salario
				_nPSalario := _a ; _nHSalario += aProve[_a,2] ; _nVSalario += aProve[_a,3]
			Endif
		Endif
	Endif
	If (_a <= Len(aDesco))
		If SRV->(dbSeek(xFilial("SRV")+aDesco[_a,1]))
			If (SRV->RV_codfol $ "054") //Falta
				_nPFaltas := _a ; _nHFaltas += aDesco[_a,2] ; _nVFaltas += aDesco[_a,3]
			Elseif (SRV->RV_codfol $ "055") //Atraso
				_nPAtraso := _a ; _nHAtraso += aDesco[_a,2] ; _nVAtraso += aDesco[_a,3]
			Endif
		Endif
	Endif
Next _a
If (_nPSalario > 0)
	aProve[_nPSalario,2] := SRA->RA_hrsmes
	If (_nVFaltas > 0)
		aProve[_nPSalario,2] -= _nHFaltas
		aProve[_nPSalario,3] -= _nVFaltas
		_nTam := Max(Len(aProve),Len(aDesco))
		For _a := 1 to _nTam
			If (_a <= Len(aDesco))
				If SRV->(dbSeek(xFilial("SRV")+aDesco[_a,1])).and.(SRV->RV_codfol $ "054") //Desconto Falta
					aDel(aDesco,_a) ; aSize(aDesco,Len(aDesco)-1)
				Endif
			Endif
		Next _a
	Endif
	If (_nVAtraso > 0)
		aProve[_nPSalario,2] -= _nHAtraso
		aProve[_nPSalario,3] -= _nVAtraso
		_nTam := Max(Len(aProve),Len(aDesco))
		For _a := 1 to _nTam
			If (_a <= Len(aDesco))
				If SRV->(dbSeek(xFilial("SRV")+aDesco[_a,1])).and.(SRV->RV_codfol $ "055") //Desconto Atraso
					aDel(aDesco,_a) ; aSize(aDesco,Len(aDesco)-1)
				Endif
			Endif
		Next _a
