...
ฑฑบPrograma  ณ PRENOTAบ Autor ณ              บ Data ณ 29/10/10 บ           ฑฑ
...
Do While .T.
	...
	aadd(aCabec,{"F1_CHVNFE" ,Substr(OId:TEXT,4,44),Nil,Nil}) //Fernando
		...
		Else
			If SF1->F1_DOC == Right("000000000"+Alltrim(OIdent:_nNF:TEXT),9)
			   ...
			   /*	If SF1->F1_PLUSER <> __cUserId
				...
				/* Desabilitado pois somente usuแrios autorizados poderใo classificar notas
			...
			cTexto   := 'A pre nota '+Alltrim(aCabec[3,2])+' Serie: '+Alltrim(aCabec[4,2]) +' do tipo '+Alltrim(aCabec[1,2]) + ' do Fornecedor '+ Alltrim(aCabec[6,2])+' Loja ' + Alltrim(aCabec[7,2]) + ' foi gerada com sucesso pelo Usuแrio '+ aInfo[1,4] + '. '+chr(13)+chr(10)+ 'Favor classificar a pre nota'
			cPara    := 'fmaia@brascola.com.br'
			cCC      := ''
			cArquivo := ''
			//fmaiaU_EnvMailto(cAssunto,cTexto,cPara,cCC,cArquivo) //para que seja enviado um arquivo em anexo o arquivo deve estar dentro da pasta protheus_data
		    U_SendMail(cPara,cAssunto,cTexto,"",.f.)
			Else
