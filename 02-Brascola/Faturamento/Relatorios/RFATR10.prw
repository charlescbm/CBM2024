...
±±º Programa    ³ RFATR10  ³ Relatório de pedidos rejeitados.                             º±±
±±º             ³          ³                                                              º±±
±±º             ³          ³                                                              º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±º Solicitante ³ 17.10.05 ³ Michele                                                      º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±º Autor       ³ 18.10.05 ³ Almir Bandina                                                º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Local cDesc2	:= "estão com pendências financeiras de acordo com seu status."
Local cDesc3	:= ""
Local cString	:= "SC9"
...
aAdd(aRegs,{ cPerg,"15","Tipo Relatório",		"","","mv_chf","N",01,0,0,"C","","MV_PAR15","Analitico","","","",				"","Sintetico",	"","","","","","","","","","","","","","","","","","","",  	"","","","","" } )
...
// Verifica se o usuario for um vendedor, forçar filtrar apenas os pedidos dele
////dbSelectArea("SA3")
////dbSetOrder(7)
...
For iX:= 1 to Len( aDadImp )
	...
	If cVendAtu <> aDadImp[ix][1]
		...
		If nTotVend > 0
			...
			// Zera array para o próximo vendedor
			aResVend	:= {}
	...
	If mv_par15 == 1  	// Analitico
		...
		@ nlin, 132 Psay StoD( aDadImp[ix][8] )//Emissão
    ...
    /*
	//Monta array com o resumo por motivo de rejeição do vendedor
...
// Imprime os dados do último vendedor
If mv_par15 == 2 .and. nTotPedi > 0
	@ nlin, 000 Psay cCliAtu+"-"+cLojAtu
	@ nlin, 010 Psay cRazAtu
	@ nlin, 031 Psay cPedAtu
...
If nTotVend > 0
	...
	// Imporime o resumo do último vendedor
	If Len( aResVend ) > 0
		// Imprime o resumo
		@ nlin,(Limite/2)-30 Psay "Resumo do Vendedor "+ cVendAtu
		nlin++
...
// ATENÇÃO - não se pode mudar a ordem so select, pois é utilizado na impressão a mesma ordem.    
...
// Verifica os pedidos que são de pagamento antecipado e trata o array
...
// Não considerar os motivos do sistema
