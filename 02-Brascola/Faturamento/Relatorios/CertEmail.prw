					...
					WHILE !EOF() .AND. ALLTRIM(SD2->D2_DOC) == ALLTRIM(SF2->F2_DOC) .AND. ALLTRIM(SD2->D2_SERIE) == ALLTRIM(SF2->F2_SERIE)						
						...
						If QP6->(DbSeek( xFilial("QP6") + SD2->D2_COD)) .AND. QP6->QP6_SKPLOT <> '27'
							...
							//If SC2->(DbSeek(SD2->D2_LOTECTL))
								...
								DbSelectArea("QPR")//MediÁıes ñ Dados GenÈricos
...
_cBody    :="<HTML>"
_cBody    += "<HEAD><TITLE> InconsistÍncias encontradas na emiss„o de certificados da qualidade</TITLE></HEAD>"
_cBody    += "<BODY>"
_cBody    +="<p style=line-height: 100%; margin-top: 0; margin-bottom: 0></p>"
_cBody    +="<P> </P>"
_cBody    +="<P> </P>"
_cBody    +="<P> </P>"
_cBody    +="<P> Favor verificar inconsistÍncias encontradas abaixo" + "</P>"
_cBody    +="<P> </P>"
_cBody    +="<P> </P>"
...
_cBody    += '    <td align=center width=350 height=15><font face="Arial Black" size=1>InconsistÍncia</font></td>'
...
_cBody    +="<P> </P>"
_cBody    +="<P> </P>"
_cBody    +="<P> E-mail gerado pelo mÛdulo Quality em " + dtoc(ddatabase) + "as " + Left(Time(),5) + "</P>"
_cBody    += "</BODY>"
...
If lOk
	...
	TO _cResp ;
	SUBJECT 'InconsistÍncia na emiss„o de Certificados de Qualidade' ;
	BODY _cBody ;
	RESULT lOk
	If !lOk
...
While !EOF()
   ...
   ConOut("----------------------------------------")
	...
	_cBody += "</body>"
	_cBody += "<body>"                                                                                                                                                              
	_cBody += "   <tr><font face='Tahoma' size='2'><b>FabricaÁ„o</b>: "+DtoC(RESX->FABRI)+" </font></tr>" 
	_cBody += "</body>"
	_cBody += "<body>"                                                                                                                                                              
	...
	_cBody += "      <td width='40%' align='left' bgcolor='#f0f0f0'> <font face='Tahoma' size='2' ><b>EspecificaÁ„o</font></b></td>"
...
If !lEnvCertif //nao enviou nenhum certificado.
	...
	_cBody += "</body>"
	_cBody += " <hr> " //linha horizontal
	_cBody += '<p align="left"><font face="Tahoma" size="1"><i>NotificaÁ„o emitida automaticamente em '+DtoC(Date())+' ‡s '+Time()+' hs.</i></font></p>'
	_cBody += "<body>"
	_cBody += "   <td><font face='Tahoma' size='1' color='#ffffff' >.</font></td>"                                                                                        
	_cBody += "</body>"
	_cBody += "</html>"

	_cAssunto:= "NotificaÁ„o sobre certificado de qualidade - "+DtoC(dDataBase)
