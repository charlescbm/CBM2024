...
//oBrwFat:AddColumn(TCColumn():New("Filial"    , {|| aFatura[oBrwFat:nAt,01]},,,,,20,.F.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Pedido"    , {|| aFatura[oBrwFat:nAt,02]},,,,,20,.F.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Item"      , {|| aFatura[oBrwFat:nAt,03]},,,,,15,.F.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Produto"   , {|| aFatura[oBrwFat:nAt,04]},,,,,30,.F.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Descrição" , {|| aFatura[oBrwFat:nAt,05]},,,,,80,.F.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Qtd. Est." , {|| aFatura[oBrwFat:nAt,06]},"@E 9,999,999.99",,,, 30 ,.F.,.T.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Saldo Disp", {|| aFatura[oBrwFat:nAt,07]},"@E 9,999,999.99",,,, 30 ,.F.,.T.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Quantidade", {|| aFatura[oBrwFat:nAt,08]},"@E 9,999,999.99",,,, 40 ,.F.,.T.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Valor"     , {|| aFatura[oBrwFat:nAt,09]},"@E 9,999,999.99",,,, 40 ,.F.,.T.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Desconto"  , {|| If(aFatura[oBrwFat:nAt,10] == 'B',oNo,oOk) },,,,,30,.T.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Liberação" , {|| If(aFatura[oBrwFat:nAt,11] == 'X',oAt,oOk) },,,,,30,.T.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Crédito"   , {|| If(Empty(aFatura[oBrwFat:nAt,12]),oOk,If(aFatura[oBrwFat:nAt,12]=='09',oNo,oAt))},,,,,30,.T.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Estoque"   , {|| If(aFatura[oBrwFat:nAt,13] == 'B',oAt,oOk) },,,,,30,.T.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Lib.Pedido", {|| aFatura[oBrwFat:nAt,14]},,,,,30,.F.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Cliente"   , {|| aFatura[oBrwFat:nAt,15]},,,,,80,.F.,.F.,,,,.F., ) )
...
//////////////////////////////////////////
If (cOpcao $ "13") //Pendentes de Liberacao
	cQuery1 := "SELECT SC6.C6_FILIAL AS FILIAL, SA1.A1_NOME AS NOME, SC6.C6_NUM AS PEDIDO, SC6.C6_ITEM AS ITEM, SC6.C6_PRODUTO AS PRODUTO, SC6.C6_LOCAL AS ARMAZEM,"
	...
	cQuery1 += "(SC6.C6_QTDVEN-(SC6.C6_QTDEMP+SC6.C6_QTDENT)) AS QUANT,SC6.C6_X_BLDSC AS BLQDESCO,'01' AS BLQCREDITO,'' AS BLQESTOQUE,'X' AS BLQPEDLIB, "
	cQuery1 += "(CASE WHEN (SC6.C6_QTDVEN-(SC6.C6_QTDEMP+SC6.C6_QTDENT))>(SB2.B2_QATU-SB2.B2_RESERVA) THEN 'B' ELSE '' END) AS BLQESTDISP "
	...
	While !MAR->(Eof())
		...
		AADD(aFatura,{MAR->FILIAL,MAR->PEDIDO,MAR->ITEM,MAR->PRODUTO,MAR->DESCRI,MAR->QTDEST,MAR->SALDO,;
			MAR->QUANT,MAR->VALOR,MAR->BLQDESCO,MAR->BLQPEDLIB,MAR->BLQCREDITO,MAR->BLQESTDISP,iif(MAR->TIPLIB=="1","Item","Pedido"),MAR->NOME})
...
If (cOpcao != "3") //Diferente de Pendente de Liberacao
	cQuery1 := "SELECT SC9.C9_FILIAL AS FILIAL,SA1.A1_NOME AS NOME,SC9.C9_PEDIDO AS PEDIDO,SC9.C9_ITEM AS ITEM,SC9.C9_PRODUTO AS PRODUTO,"
	cQuery1 += "SC9.C9_LOCAL AS ARMAZEM,SB2.B2_QATU AS QTDEST,(SB2.B2_QATU-SB2.B2_RESERVA) AS SALDO,SB1.B1_DESC AS DESCRI,(SC9.C9_QTDLIB*SC9.C9_PRCVEN) AS VALOR," 
	...
	cQuery1 += "(CASE WHEN SC9.C9_QTDLIB>(SB2.B2_QATU-SB2.B2_RESERVA) AND SC9.C9_BLEST <> '' THEN 'B' ELSE '' END) AS BLQESTDISP "
	...
	EndIf
	If !Empty(cCliente)
		cQuery1 += "AND SC9.C9_CLI = '"+cCliente+"' "
	EndIf
	If !Empty(cLoja)
	...
	Elseif (cOpcao == "5") //Itens Sem Estoque Disponivel
		cQuery1 += "AND SC9.C9_BLEST <> '' AND SC9.C9_QTDLIB>(SB2.B2_QATU-SB2.B2_RESERVA) "
	...
	While !MAR->(Eof())
		...
		AADD(aFatura,{MAR->FILIAL,MAR->PEDIDO,MAR->ITEM,MAR->PRODUTO,MAR->DESCRI,MAR->QTDEST,MAR->SALDO,MAR->QUANT,MAR->VALOR,MAR->BLQDESCO,;
			MAR->BLQPEDLIB,MAR->BLQCREDITO,MAR->BLQESTDISP,iif(!Empty(MAR->BLQESTOQUE),iif(MAR->TIPLIB=="1","Item","Pedido"),""),MAR->NOME})
	
		nTotPed += MAR->VALOR
		If (MAR->BLQDESCO == "B") //Bloqueio Desconto
			nTotDes += MAR->VALOR
		Endif
		If (MAR->BLQCREDITO == "09") //Rejeitado Credito
			nTotRej += MAR->VALOR
		Endif
		If Empty(MAR->BLQCREDITO).and.(MAR->BLQESTOQUE == "02") //Pedidos Bloqueados Estoque
			nTotEst += MAR->VALOR
		Endif
		If (MAR->BLQCREDITO == "01") //Pendente Analise de Credito
			nTotCre += MAR->VALOR
		Endif
		If (MAR->BLQESTDISP == "B") //Item Estoque Disponivel
			nTotSes += MAR->VALOR
		Endif
		If Empty(MAR->BLQCREDITO).and.Empty(MAR->BLQESTOQUE) //Apto a Faturar
			nTotFat += MAR->VALOR
		Endif
...
For nx := 1 to 2 //1=Calcula para o Mes / 2=Calcula para o Dia
	...
	cQuery1 += "WHERE SD2.D_E_L_E_T_='' AND SF4.F4_DUPLIC = 'S' "
	If (nx == 1)
...
TRCell():New(oSection1,"C6_PRODUTO","SC6","Produto")
...
oSection1:Cell("C6_PRODUTO"):SetSize(30)
...
cFiltro1 := "AND SC6.C6_ENTREG BETWEEN '" + DTOS(dEntDe) + "' AND '" + DTOS(dEntAte) + "' "
cFiltro2 := "AND SC6.C6_ENTREG BETWEEN '" + DTOS(dEntDe) + "' AND '" + DTOS(dEntAte) + "' "
