...
oGetFinBco := tGet():New(030,190,{|u| if(PCount()>0,cBcoFin:=u,cBcoFin)}, oGrpFilFin,50,9,'@!',{ ||  },,,,,,.T.,,, {|| .T. } ,,,,.F.,,'SA6','cBcoFin')

oSayFinAge := tSay():New(032,250,{ || "Agência:" },oGrpFilFin,,,,,,.T.,CLR_BLACK,CLR_WHITE,50,9)
oGetFinAge := tGet():New(030,275,{|u| if(PCount()>0,cAgeFin:=u,cAgeFin)}, oGrpFilFin,50,9,'@!',{ ||  },,,,,,.T.,,, {|| .T. } ,,,,.F.,,,'cAgeFin')

oSayFinCta := tSay():New(032,335,{ || "Conta:" },oGrpFilFin,,,,,,.T.,CLR_BLACK,CLR_WHITE,50,9)
oGetFinCta := tGet():New(030,355,{|u| if(PCount()>0,cCtaFin:=u,cCtaFin)}, oGrpFilFin,50,9,'@!',{ ||  },,,,,,.T.,,, {|| .T. } ,,,,.F.,,,'cCtaFin')
...
Do Case
	...
	Case cFilFat == "00"
		cFilFil := "AND SD2.D2_FILIAL IN ('01','03') "
		cFilDev := "AND SD1.D1_FILIAL IN ('01','03') "
EndCase
...
BeginSql alias cAlias
	...
	(
	SELECT SUM(D1_TOTAL+D1_ICMSRET+D1_VALIPI-D1_VALDESC) AS DEV_MES
	FROM %table:SD1% SD1
   ...
   //	AND SD1.D1_TIPO = 'D'
	...
	(
	SELECT SUM(D1_TOTAL+D1_ICMSRET+D1_VALIPI-D1_VALDESC) AS DEV_DIA
	FROM  %table:SD1% SD1
