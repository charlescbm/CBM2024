...
Private aOpcao   := {"1=Todos","2=Pend. Liberação","3=Rej. Crédito","4=Bloq. Estoque","5=Pedidos sem Estoque","6=Pend. Anal. Créd.","7=Aptos a Faturar"}
...
oSayOpcP := tSay():New(007,005,{|| "Opções:" },oFolder:aDialogs[3],,,,,,.T.,CLR_BLACK,CLR_WHITE,50,9)
...
oGrpTotMes := tGroup():New(175,005,255,220,"Pedidos que entraram no mês " + STRZERO(MONTH(dDataEnt),2) +"/" + ALLTRIM(STR(YEAR(dDataEnt))), oFolder:aDialogs[3],CLR_HBLUE,,.T.)
...
oSayFinAge := tSay():New(032,250,{ || "Agência:" },oGrpFilFin,,,,,,.T.,CLR_BLACK,CLR_WHITE,50,9)
...
oBrwFat:AddColumn(TCColumn():New("Liberação" , {|| If(aFatura[oBrwFat:nAt,09] == 'X',oAt,oOk) },,,,,25,.T.,.F.,,,,.F., ) )
oBrwFat:AddColumn(TCColumn():New("Crédito"   , {|| If(aFatura[oBrwFat:nAt,10] == 'X',oNo,If(aFatura[oBrwFat:nAt,10] == 'A',oAt,oOk)) },,,,,25,.T.,.F.,,,,.F., ) )
...
oBrwPed:AddColumn(TCColumn():New("Desc. Médio"   , {|| aPedidos[oBrwPed:nAt,06]},"@R 99.99%",,,,40,.F.,.F.,,,,.F., ) )
...
While !Eof()
	...
	IncProc("Buscando informações de entrada de pedidos...")
...
oBrwPed:AddColumn(TCColumn():New("Desc. Médio"    , {|| aPedidos[oBrwPed:nAt,02]},"@R 99.99%",,,,40,.F.,.F.,,,,.F., ) )
oBrwPed:AddColumn(TCColumn():New("Entrega Mês"    , {|| aPedidos[oBrwPed:nAt,03]},"@E 9,999,999.99",,,,40,.F.,.F.,,,,.F., ) )
oBrwPed:AddColumn(TCColumn():New("Qtd. Ent. Mês"  , {|| aPedidos[oBrwPed:nAt,04]},"@E 9,999,999.99",,,,40,.F.,.F.,,,,.F., ) )
...
Else //Charles Battisti Medeiros  
	...
	{"&Ok"},,"Customização - TI - Brascola Ltda. ")  //Charles Battisti Medeiros  
