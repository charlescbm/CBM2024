...
If cTipo == "1"
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
				...
				If !Empty(SF4->F4_MENNOT1) .And. aScan(xCOD_MENS, SF4->F4_MENNOT1) == 0 .AND. SM0->M0_CODFIL $ '01*02*03*08'
					aAdd(xCOD_MENS, SF4->F4_MENNOT1)
				EndIf                  
				If !Empty(SF4->F4_MENNOT2) .And. aScan(xCOD_MENS, SF4->F4_MENNOT2) == 0 .AND. SM0->M0_CODFIL = '04'
					aAdd(xCOD_MENS, SF4->F4_MENNOT2)
				EndIf
				*/
				//================================================ FIM CUSTOMIZACAO ===============================================//
					...
					/* Marcelo - 12/08/13 - Comentado para n„o duplicar
					...
					*/
					...
					aadd(aProd,	{Len(aProd)+1,;
						...
						SB1->B1_UM,;
						(cAliasSD2)->D2_QUANT,;
						IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCZFR,IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;
						IIF(Empty(SB5->B5_UMDIPI),SB1->B1_UM,IIF(SA1->A1_UMFAT =="1",SB1->B1_UM,SB5->B5_UMDIPI)),; 
						IIF(Empty(SB5->B5_CONVDIPI),(cAliasSD2)->D2_QUANT,IIF(SA1->A1_UMFAT =="1",(cAliasSD2)->D2_QUANT,SB5->B5_CONVDIPI*(cAliasSD2)->D2_QUANT)),;
						(cAliasSD2)->D2_VALFRE,;
						(cAliasSD2)->D2_SEGURO,;
						(nDesconto+nDescIcm+nDescRed),;
						IIF(!(cAliasSD2)->D2_TIPO$"IP",NoRound( ( (cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCZFR )/(cAliasSD2)->D2_QUANT,8),0 ),;
				...
				dbSelectArea("SA1")
				dbSetOrder(1)
			    dbSeek(xFilial("SA1")+(cAliasSD2)->(D2_CLIENTE+D2_LOJA))
				If SA1->A1_INTNFE == 'S' 
	            	_pedCLI:=Posicione("SC6", 1, xFilial("SC6") + (cAliasSD2)->D2_PEDIDO + (cAliasSD2)->D2_ITEMPV, "C6_PEDCLI" ) 
	            	aadd(aPedCom,{_pedCLI,(cAliasSD2)->D2_ITEMPV}) 
	            else
	            	aadd(aPedCom,{})
				ENDIF	
					aadd(aPisAlqZ,{})
					...
					//28/04/14 - Conforme chamado 6822 - Silvia
					///////////////////////////////////////////
					nICMSCom := 0
					///////////////////////////////////////////
					...
					While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
					   ...
					   nMargem :=  IiF(CD2->CD2_PREDBC>0,IiF(CD2->CD2_PREDBC == 100,CD2->CD2_PREDBC,IF(CD2->CD2_PREDBC > 100,0,100-CD2->CD2_PREDBC)),CD2->CD2_PREDBC)              		 					
						...
						//28/04/14 - Conforme chamado 6822 - Silvia
						///////////////////////////////////////////
						If (Alltrim(CD2->CD2_IMP) == "CMP")
							nICMSCom += CD2->CD2_vltrib
						Endif
						///////////////////////////////////////////
					...
					If (nICMSCom > 0)
						If (Len(cMensCli) > 0).and.(Substr(cMensCli,Len(cMensCli),1) <> " ")
							cMensCli += " "
						Endif
						cMensCli += "ICMS Complementar: "+Alltrim(GetMV("MV_SIMB1"))+" "+Alltrim(Transform(nICMSCom,PesqPict("CD2","CD2_VLTRIB")))+"."
					Endif
					///////////////////////////////////////////
				...
				If MsSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)			
					If !SF3->F3_DESCZFR == 0 .or. ( lInfAdZF .and. nValBse > 0 )
						If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
						   cMensFis += " "
						EndIf					
						If lInfAdZF .And. (nValPisZF > 0 .Or. nValCofZF > 0)    // -- ALTERADO POR CHARLES
							...
							nZfPis := ((SF2->F2_VALMERC + SF2->F2_DESCZFR)*(1.65/100))
							nZfCof := ((SF2->F2_VALMERC + SF2->F2_DESCZFR)*(7.60/100))
							//cMensFis += "Descontos Ref. a Zona Franca de Manaus / ALC. ICMS - R$ "+str(SF2->F2_DESCZFR-nValPisZF-nValCofZF,13,2)+", PIS - R$ "+ str(nValPisZF,13,2) +"e COFINS - R$ " +str(nValCofZF,13,2) 											
							cMensFis += "Descontos Ref. a Zona Franca de Manaus / ALC. ICMS - R$ "+str(SF2->F2_DESCZFR-nValPisZF-nValCofZF,13,2)+", PIS - R$ "+ str(nZfPis,13,2) +"e COFINS - R$ " +str(nZfCof,13,2) 											
						ElseIF !lInfAdZF .And. (nValPisZF > 0 .Or. nValCofZF > 0) 
							...
							cMensFis += "Desconto Ref. ao ICMS - Zona Franca de Manaus / ALC. R$ "+str(SF2->F2_DESCZFR-nValPisZF-nValCofZF,13,2)
					    ElseIF (SF2->F2_DESCZFR) > 0
					    	//cMensFis += "Total do desconto Ref. a Zona Franca de Manaus / ALC. R$ "+str(SF3->F3_VALOBSE-SF2->F2_DESCONT,13,2)  
					    	cMensFis += "Total do desconto Ref. a Zona Franca de Manaus / ALC. R$ "+str(SF2->F2_DESCZFR,13,2)	
					    EndIF
					EndIf 			
				EndIf	
			EndIF
				...
				ElseIf !lEasy
					...
					If MsSeek(xFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
							aAdd(aDI,{;
			      			...
			      			// MARCELO - 20/01/2014 - DADOS DA DI
			      			/////////////////////////////////////
								If Len(cMensCli) > 0 .And. SubStr(cMensCli, Len(cMensCli), 1) <> " "
									cMensCli += " "
								EndIf
				            cMensCli += " - ValAdu: "+Alltrim(Transform(CD5->CD5_DSPAD,"@E 999,999,999.99"))
				            cMensCli += " - ValII: "+Alltrim(Transform(CD5->CD5_VLRII,"@E 999,999,999.99"))
				            cMensCli += " - ValPis: "+Alltrim(Transform(CD5->CD5_VLPIS,"@E 999,999,999.99"))
				            cMensCli += " - ValCof: "+Alltrim(Transform(CD5->CD5_VLCOF,"@E 999,999,999.99"))
				            cMensCli += " - ValDes: "+Alltrim(Transform(SF1->F1_DESPESA,"@E 999,999,999.99"))
				            cMensCli += " - Total: "+Alltrim(Transform(SF1->F1_VALMERC,"@E 999,999,999.99"))
				            cMensCli += " - DocImp: "+Alltrim(CD5->CD5_DOCIMP)   
				            cMensCli += " - DI: "+Alltrim(CD5->CD5_NDI)
				            cMensCli += " - Invoice: "+Alltrim(CD5->CD5_ACDRAW)
				            cMensCli += " - LocDes: "+Alltrim(CD5->CD5_LOCDES)+"-"+CD5->CD5_UFDES
			      			/////////////////////////////////////
		    ...
		    Do Case
		    	Case cCompImp == "1"
					...
					If Len(aEIC)==0
						dbSelectArea("SD1")
						dbSetOrder(1)
						dbSeek(xFilial("SD1")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA)
						If !Empty(AllTrim(SD1->D1_CONHEC)) .and. (cAliasSD1)->D1_TIPO <> 'C'
							cMensFis += "Numero DI :" +  AllTrim((cAliasSD1)->D1_CONHEC)
						EndIf
					Else
						nTotMsg := Len(aEIC)-1
						
						If Len(aEIC)>4
							For nII:=5 To nTotMsg
								cMensFis +=aEIC[nII]
							NEXT
						ENDIF
						...
						xP_LIQ_PED  := aEIC[3]
						xPESO_BRUTO := aEIC[4]
						xDESPESAS := aEIC[Len(aEIC)]
						
						If Len(aEspVol) > 0
							aEspVol:= {}
						EndIf
						
						aadd(aEspVol,{ xESPECIE ,  xVOLUME , xP_LIQ_PED , xPESO_BRUTO})
					EndIf		    	
		    	Case cCompImp == "C"
					aEIC := u_EICNFE()
					
					If Len(aEIC)==0
						cMensFis += "Nota compl. da NF :" + AllTrim((cAliasSD1)->D1_NFORI)+ "/" +AllTrim((cAliasSD1)->D1_SERIORI)
					Else
						xVOLUME     := aEIC[1]
						xESPECIE    := aEIC[2]
						xP_LIQ_PED  := aEIC[3]
						...
						aadd(aEspVol,{ cEspecie,  xVOLUME , xP_LIQ_PED , xPESO_BRUTO})
					EndIf
...
Endif
cString += '<vProd>' +ConvType(aProd[10],15,4)+'</vProd>' 
...
cString += NfeTag('<vDesc>' ,ConvType((aProd[26]),15,2))
...
If SF2->F2_EST=="EX" 
 cString += '<exporta>'
 cString += '<UFEmbarq>'+ConvType(SM0->M0_ESTENT)+ '</UFEmbarq>'
 cString += '<locembarq>'+ConvType(SM0->M0_NOMECOM)+ '</locembarq>'
 cString += '</exporta>' 
EndIf
*/
//================================================ FIM CUSTOMIZACAO ===============================================//
