...
#DEFINE MAXITEMP2 023                                                // Máximo de produtos para a pagina 2 (caso nao utilize a opção de impressao em verso)
#DEFINE MAXITEMP3 023                                                // Máximo de produtos para a pagina 2 (caso utilize a opção de impressao em verso) - Tratamento implementado para atender a legislacao que determina que a segunda pagina de ocupar 50%.
#DEFINE MAXITEMP4 023                                                // Máximo de produtos para a pagina 2 (caso contenha main info cpl que suporta a primeira pagina)
...
#DEFINE MAXBOXV   600
#DEFINE INIBOXH   -10
#DEFINE MAXMENL   100                                                // Máximo de caracteres por linha de dados adicionais
...
User Function DANFE_P1(cIdEnt,cVal1,cVal2,oDanfe,oSetup,aAutoDanfe)
...
RptStatus({|lEnd| DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe,aAutoDanfe)},"Imprimindo Danfe...")
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,cVal1,cVal2,lExistNfe,aAutoDanfe)
...
If (aAutoDanfe != Nil)
	Pergunte("NFSIGW",.F.)
	mv_par01 := aAutoDanfe[1] //Nota De
	mv_par02 := aAutoDanfe[2] //Nota Ate
	mv_par03 := aAutoDanfe[3] //Serie
	mv_par04 := aAutoDanfe[4] //Tipo Operacao
	mv_par05 := aAutoDanfe[5] //Verso
	mv_par06 := aAutoDanfe[6] //Simplificado
Else
	lContinua := Pergunte("NFSIGW",.T.) .And. ( !Empty( MV_PAR06 ) .and. MV_PAR06 == 2 .Or. ( Empty( MV_PAR06 ) )) 
Endif
If (lContinua)
	//BRASCOLA - Validacao notas conferidas
	///////////////////////////////////////
	cQuery := " SELECT F2_DOC,F2_SERIE,F2_CONFERE FROM SF2010 "
	cQuery += " WHERE F2_FILIAL = '"+xFilial("SF2")+"' "
	cQuery += " AND F2_DOC >= '"+ MV_PAR01+"' AND F2_DOC <='"+ MV_PAR02+"' "
	cQuery += " AND F2_CONFERE <> '1' " 
	If Select("TRAB5") > 0 
		TRAB5->(dbCloseArea())
	EndIf	
	TCQUERY cQuery NEW ALIAS "TRAB5"
	dbSelectArea("TRAB5")
	dbGotop() 	
   //Fernando
   cUsuario:= AllTrim(USRRETNAME(RETCODUSR()))
   cParam:= GETMV("BR_000017")//usuarios que podem imprimir Danfe sem conferencia  
	If (!cUsuario $ cParam)
		IF !EOF()//Fernando
	   		WHILE !EOF()
				_NOTA:=TRAB5->F2_DOC+"/"
				DBSKIP()
			ENDDO
	  		MSGALERT("Notas:"+_NOTA+"PRECISA SER CONFERIDA")
			RETURN
		ENDIF
	ENDIF
	///////////////////////////////////////
...
//============================================== INICIO CUSTOMIZAÇÃO ==============================================//	
	cCodProd := oDet[nX]:_Prod:_cProd:TEXT
...
ElseIf cModFrete =="1"
	//oDanfe:Say(nLine+397,412,"1-DEST/REM",oFont08:oFont) //Fernando: 07.03.12
...
//============================================== FIM CUSTOMIZAÇÃO ==============================================//
	...
	nLinha := nLinha + 10 
Next nY 
If nL <= MAXITEM .And. Len(aMensagem) > MAXMSG .And. nFolha == 2 .And. nLenItens <= MAXITEM
	oDanfe:EndPage()
	oDanfe:StartPage()
	nLinha    	:=	181                
	DanfeCab(oDanfe,nPosV,oNFe,oIdent,oEmitente,nFolha,nFolhas,cCodAutSef,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,@nLine,@nBaseCol,@nBaseTxt,aUf)
...
If nFolha == 1
	...
	nAux += aTamCol[2]
	AADD(aColProd, {nAux, nAux + aTamCol[3]}) // "N. ONU"
	nAux += aTamCol[3]
...
/*
	...
	nAux += aTamCol[16]
*/
	AADD(aColProd, {nAux, nAux + aTamCol[11]}) // "VALOR IPI"
	nAux += aTamCol[11]                   
	AADD(aColProd, {nAux, nAux + aTamCol[12]}) // "ICMS"
	nAux += aTamCol[12]
	AADD(aColProd, {nAux, nAux + aTamCol[13]}) // "IPI"
	nAux += aTamCol[13]
	AADD(aColProd, {nAux, nAux + aTamCol[14]}) // "DESC.ZFM"
	...
	oDanfe:Say(nLine+462, nAux + 2,"N. ONU", oFont08N:oFont)
	nAux += aTamCol[3]
...
*/
	...
	oDanfe:Box(nLine+454, nAux2, nLine+461, nAux2 + aTamCol[12] + aTamCol[13])
	oDanfe:Say(nLine+460, nAux2 + 2,"ALIQUOTA", oFont08N:oFont)
	nAux += aTamCol[13]
	oDanfe:Say(nLine+468, nAux + 2,"DESC.ZFM", oFont08N:oFont)
...
ElseIf nFolha == 2
 ...
 /////////////////////////////////////////////////////////////INICIO CUSTOMIZAÇÃO//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	/*
	xNumPedAx:= Posicione("SD2",3,SF2->F2_FILIAL+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA,"D2_PEDIDO")
	xTranspAx:= Posicione("SA4",1,xFilial("SA4")+AllTrim(SF2->F2_REDESP),"A4_COD")
	
	nPosVOld += Char2PixV(oDanfe,"X",oFont12)*2
	oDanfe:Say(nPosVOld,nAuxH,"DECLARAMOS QUE OS PRODUTOS ESTÃO ADEQUADAMENTE ACONDICIONADOS PARA SUPORTAR OS RISCOS NORMAIS DE CARREGAMENTOS,",oFont12:oFont)
	
	nPosVOld += Char2PixV(oDanfe,"X",oFont12)*2
	oDanfe:Say(nPosVOld,nAuxH,"DESCARREGAMENTOS, TRANSPORTE E TRANSBORDOS CONFORME REGULAMENTAÇÃO EM VIGOR, ART. 6º E ART. 22 DO DECRETO 96044 DE 18/05/88." ,oFont12:oFont)
	
	nPosVOld += Char2PixV(oDanfe,"X",oFont12A)*(1.5)
	
	nPosVOld += Char2PixV(oDanfe,"X",oFont12)*2
	oDanfe:Say(nPosVOld,nAuxH,"Pedido nº: "+xNumPedAx+"   |   Vendedor: "+SF2->F2_VEND1+"   |   N/ Numero Zona Franca: "+SA1->A1_SUFRAMA + IIf(!Empty(SF2->F2_REDESP),"   |   Redespacho: "+SA4->A4_COD+"-"+Alltrim(SA4->A4_NREDUZ)," ") ,oFont12:oFont)
	
	If !Empty(SF2->F2_REDESP)
		nPosVOld += Char2PixV(oDanfe,"X",oFont12)*2
		oDanfe:Say(nPosVOld,nAuxH,"End.Redespacho: "+AllTrim(SA4->A4_END)+", "+AllTrim(SA4->A4_BAIRRO)+", "+AllTrim(SA4->A4_MUN)+", "+AllTrim(SA4->A4_EST)+" -CGC: "+Transform(SA4->A4_CGC,"@R 99.999.999/9999-99")+" -IE:"+AllTrim(SA4->A4_INSEST) ,oFont12:oFont)
	EndIf  
	*/
  CUSTOMIZAÇÃO//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
