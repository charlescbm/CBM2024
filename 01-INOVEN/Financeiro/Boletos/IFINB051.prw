...
User Function BOLSANT01(_xNum1,_xNum2,_xPrefixo)
...
Static Function SYIMPBO01(oPrint)
    ...
    If !lImpri
	   ...
	   cDigitao	  := IIF(lReimp,SubStr(cNumBoleto,Len(cNumBoleto),1),BRPDIG2( cAgencia + cNumConta + cCarteira + StrZero(Val(cNumBoleto),8),cNumBoleto ))
	     ...
	     DbSelectArea("SE1")
	     RecLock("SE1",.F.)
	       SE1->E1_NUMBCO  := StrZero(Val(ALLTRIM(cNumBoleto)),10)
	...
	cNumBoleto := StrZero(Val(cNumBoleto),13)
	
	cDadosCta	:=	cAgencia +  " / " + cCodCed
	cDigCodBar	:=	Modulo11(cNumBanco + "9" + cFatorVcto + cValorTit + '9' + cCodCed + cNumBoleto + '0' + cCarteira )
	cDigCodBar	:= IIF(cDigCodBar $ "0|1|10","1",cDigCodBar)
	cCodBar	    := cNumBanco + "9" + cDigCodBar + cFatorVcto + cValorTit + '9' + cCodCed + cNumBoleto + '0' + cCarteira 
	...
	cDigblc2	:=	Modulo10(SubStr(cCodCed,5,3) + SubStr(StrZero(val(cNumBoleto),13),1,7))
	cBloco2		:= 	Transform(SubStr(cCodCed,5,3) + SubStr(StrZero(val(cNumBoleto),13),1,7) + cDigblc2,"@R 99999.999999")
	cDigblc3	:=	Modulo10(SubStr(StrZero(val(cNumBoleto),13),8,6) + '0' + cCarteira)
	cBloco3		:=	Transform(SubStr(StrZero(val(cNumBoleto),13),8,6) + '0' + cCarteira + cDigblc3,"@R 99999.999999")
