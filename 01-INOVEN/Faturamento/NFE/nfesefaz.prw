...
Private aUF     	:= {}
Private aCSTIPI 	:= {}
Private nDias := GETMV("TG_NDIASAI")// INOVEN
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
				...
				While ( !Empty(cScan) )
					...
					If !empty(cEspecie)
						...
						If ( nScan==0 .AND.cScan == "1" )
							...
							EndDo
							if !empty(nCuba)
								cNumeracao := Transform(nCuba,"@E 999999.9999")
							endif*/
							if !empty(SF2->F2_ZCUBA)
								cNumeracao := Transform(SF2->F2_ZCUBA,"@E 999999.99999")
							endif
							//FIM - Cubagem
				...
				If !Empty(SF2->F2_DUPL)	.Or. lVLojaDir
					...
					/*Ajuste da Prorrogacao do titulo - ESPECIFICO INOVEN*/
					if GETMV("TG_NDIASAI") > 0 .and. empty(SF2->F2_CHVNFE)
					...
					#IFDEF TOP
						...
						BeginSql Alias cAliasSE1
							...
							SELECT E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCREA E1_VENCORI,E1_VALOR,E1_VLCRUZ,E1_ORIGEM,E1_PIS,E1_COFINS,E1_CSLL,E1_INSS,E1_VLRREAL,E1_IRRF,E1_ISS,E1_NATUREZ
							FROM %Table:SE1% SE1
							WHERE
				...
				If !Empty((cAliasSD1)->D1_IDENTB6) .And. lNFPTER  
					If (cAliasSD1)->D1_TIPO == "N" 
						...
						If SA5->(MsSeek( xFilial("SA5") + (cAliasSD1)->(D1_FORNECE+D1_LOJA+D1_COD) )) .and. !empty(SA5->A5_CODPRF) .and. !empty(SA5->A5_DESREF)
							cCodProd  := SA5->A5_CODPRF 
							cDescProd := alltrim(SA5->A5_DESREF)
						EndIf
					ElseIf (cAliasSD1)->D1_TIPO == "B"
			         //--A7_FILIAL + A7_CLIENTE + A7_LOJA + A7_PRODUTO
						...
						If SA7->(MsSeek( xFilial("SA7") + (cAliasSD1)->(D1_FORNECE+D1_LOJA+D1_COD) )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI) 
							cCodProd  := SA7->A7_CODCLI 
							cDescProd := alltrim(SA7->A7_DESCCLI)
						EndIf
					EndIf
				EndIf			
...
cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(DATAVALIDA(aNota[03]+ nDias ))+"T"+Iif(Len(AllTrim(aNota[06])) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00"))) // Altera data de saida - INOVEN
